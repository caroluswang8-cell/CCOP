# Strict SPH2 configuration-assignment comparison

The driver `card_piola_pressure_matrixfree_jax_v12_sph2_three_branch_strict.py`
preserves the accepted corrected second-order material SPH discretization and
adds only two matched
...
- ring cloud, material free-surface pressure nodes, corrected SPH2;
- mandatory fp64;
...
$code = '.\code\card_piola_pressure_matrixfree_jax_v12_sph2_three_branch_strict.py'
```

Reference CCOP midpoint branch:

```powershell
& $py -u $code --backend cpu --branch ccop-midpoint --output sph2_ccop_midpoint_Rdx50_dt0p002_T30.npz
```

Matched predictor-configuration control:

```powershell
& $py -u $code --backend cpu --branch star-star --output sph2_star_star_Rdx50_dt0p002_T30.npz
```

Terminal-action control:

```powershell
& $py -u $code --backend cpu --branch terminal-terminal --output sph2_terminal_terminal_Rdx50_dt0p002_T30.npz
```
