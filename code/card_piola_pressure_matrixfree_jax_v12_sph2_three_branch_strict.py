"""
Strict SPH2 three-branch configuration-assignment comparison.

The original SPH2 solver is preserved separately.  This comparison driver keeps
its particles, accepted second-order corrected material SPH derivative,
pressure boundary space,
...
class PiolaPressureSolver:
    def __init__(self, args):
        ...
        self.neigh_np = neigh
        self.use_sph2_derivative_operator = args.reference_operator in (
            'corrected', 'corrected-sph', 'corrected-sph2', 'sph2'
        )

        if self.use_sph2_derivative_operator:
            deriv = build_mls_derivative_operator(X, self.h, 2, args.mls2_reg)
            self.deriv_np = deriv
            print(f'[reference-operator] corrected-sph2: quadratic MLS/SPH derivative weights, '
                  f'maxn={int(deriv["maxn"][0])}, '
        ...
        if (not self.use_sph2_derivative_operator
                or args.reference_operator not in ('corrected', 'corrected-sph', 'corrected-sph2', 'sph2')):
            raise RuntimeError('The comparison is locked to the accepted corrected SPH2 operator.')
        ...
        print('=== Strict SPH2 CCOP configuration-assignment comparison ===')
    ...
    def _build_material_derivative_sparse(self):
        ...
        print(f'[SPH2] sparse derivative constant defect={const_defect:.3e}')
        return Dx, Dy
    ...
    def _sph_F(self, x: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    ...
    def _compute_state_from_p(self, p_u: jnp.ndarray, x_n: jnp.ndarray, u_star: jnp.ndarray,
        ...
        def body(_, xT_inner):
            ...
            _, invFtheta, _ = self._sph_F(x_theta)
...
def main():
    ap = argparse.ArgumentParser(
        description='Strict SPH2 comparison: D_T/G_1/2, D_star/G_star, or D_T/G_T.'
    )
    ...
    ap.add_argument('--mls2-reg', type=float, default=1e-12,
                    help='Moment regularization for the accepted corrected SPH2 derivative weights.')
    ap.add_argument('--reference-operator',
                    choices=['raw', 'corrected', 'corrected-sph', 'corrected-sph2', 'sph2'],
                    default='corrected-sph2',
                    help='Strict comparison is locked to corrected-sph2 (aliases are accepted).')
    ...
    if not args.output:
        args.output = f'sph2_{args.branch.replace("-", "_")}_Rdx50_dt0p002_T30.npz'
