...
sets the terminal constraint geometry, and
\cref{sec:finite_step_action_work} develops action placement and pressure work.
\Cref{sec:ccop_analysis} derives the cross-configuration pressure response and
its Schur projector.
...
space.  Its consistency with $D_T^{\rm str}$ and with the composite response
$D_T^{\rm str}\mathcal G_\Th$ is defined in
\cref{sec:discrete_operator_consistency}.  This separation preserves the
state--tangent interpretation while allowing the numerical experiments to
measure spatial reconstruction and finite-step state error independently.
...
\section{Finite-step pressure action and work}
\label{sec:finite_step_action_work}

\subsection{Geometric action placement and impulse accuracy}
...
and geometric-placement contributions.  Exact discrete pressure work follows
from a different identity established below.

\subsection{Local branch and configuration-placement defect}
...
Proposition~\ref{prop:placed_impulse_order} therefore gives a pressure-substep
design rule under stage consistency.  Second-order midpoint stage accuracy
produces a third-order local impulse defect, while an $O(\dt)$ stage error
produces an $O(\dt^2)$ local defect.  Varying $\Th$ changes the action geometry
within the same one-stage scalar pressure solve.

\subsection{Pressure work--energy identity}
...
\end{equation}
This one-point path parameterizes admissible pressure-operator geometries
independently of the state time integrator.  Hence $\Th=1/2$ defines the
