...
\begin{abstract}
Standard semi-implicit Lagrangian projection methods evaluate the incompressibility constraint and pressure action on a single geometric stage. We show that this common-stage construction hides a more fundamental configuration-assignment problem: the accepted velocity constraint belongs to the terminal configuration, whereas pressure work depends on the configuration on which the pressure impulse is represented. We therefore identify configuration consistency as a structural design principle for Lagrangian projection, separating the geometry that determines represented terminal incompressibility from that governing pressure work. We formulate this principle as configuration-consistent oblique projection (CCOP). The constraint is anchored to the terminal configuration, while the pressure action is placed independently along the material path. The cross response \(D_TG_\Theta\) retains a single scalar pressure equation and induces a configuration-dependent oblique projector. We establish its projection structure and relative-deformation solvability. For the actual force-free CCOP step on a fixed spatial discretization, a local expansion shows that midpoint action placement cancels the leading configuration-induced velocity defect without an additional pressure stage. Affine tests isolate terminal-constraint and pressure-work errors and show that identical represented closure can coexist with different correction ranges and kinetic work. A same-discretization ISPH comparison demonstrates their long-time consequences. A strictly nonaffine differential vortex then verifies the transported operators, cross response, and complete evolution under spatially varying material shear. These results establish CCOP as a configuration-consistent projection architecture that exposes and controls the finite-step relation between terminal closure and pressure work while retaining a single scalar pressure solve.
\end{abstract}
...
A related separation appears in constrained mechanics. Discrete mechanics and variational integrators treat forces and constraints as distinct ingredients of the discrete update \cite{MarsdenWest2001}; RATTLE and discrete null-space methods similarly organize constraint enforcement within structure-preserving time integration \cite{Andersen1983,Betsch2005}; and related ideas have been used in incompressible particle and geometric fluid discretizations \cite{SuzukiKoshizukaOka2007,PavlovEtAl2011,GawlikEtAl2011}. Our construction is conceptually aligned with this separation, but remains within the classical pressure-projection/PPE architecture. The specific issue here is geometric: once incompressibility is assigned to the terminal configuration, the pressure multiplier must still be represented on an appropriate material configuration. CCOP makes this configuration assignment explicit while retaining a scalar pressure solve.
...
\eqref{eq:intro_projector} is consequently the Schur-complement realization of
the method, not an auxiliary geometric construction.

The distinction exposes a geometric degree of freedom without enlarging the
pressure space.  We restrict the action to one real material configuration on the
...
especially useful: it introduces
no additional pressure stage, but under stage-consistent conditions it cancels
the leading one-point pressure-impulse defect.  The numerical comparison is
therefore not between two different spatial methods; it is between pressure
architectures that use the same terminal target but represent the finite-step
multiplier action differently.

The contribution is the configuration assignment and its computable
consequences, rather than a new algebraic identity for projectors.  The analysis
links relative deformation to cross-response solvability, correction-range
variation to projector obliqueness, and a stage defect to pressure-impulse
accuracy.  A fixed-space expansion of the actual force-free CCOP equations
...
that error to the corrected action.  The comparisons
use shared spatial operators within each case; the native-pressure variant
used for the ISPH comparison is specified separately from the cofactor-action
realization in the other cases.

The remainder is organized as follows.  \Cref{sec:configuration_assignment}
sets the terminal and action configurations.  \Cref{sec:ccop_analysis} derives
the cross-configuration pressure response and its Schur projector.
...
\section{Configuration assignment in a finite material step}
\label{sec:configuration_assignment}

\subsection{A same-configuration reference pair}
...
\end{equation}
The material-cofactor pressure action used in the following continuum
formulation and in Cases~1 and~3 is
\begin{equation}
  \G_\alpha q
...
\end{equation}
Case~2 retains the native physical-pressure gradient of its SPH2/MLS2
implementation, without the factor $J_\alpha$; its convention and matched
ISPH comparison are specified in \cref{eq:case2_physical_action}.
...
\end{equation}
This is the same-configuration Hodge reference under compatible boundary
conditions.  The meshless realization below measures its endpoint baseline
and the additional placement-dependent obliqueness separately.

\subsection{Finite-volume state and its terminal tangent}
...
This identity is continuous.  The meshless discretization does not introduce
a discrete volume functional $\mathcal C_{T,h}$ and does not assert
$D\mathcal C_{T,h}=D_{T,h}^q$.  Instead, let $I_h$ denote particle sampling and
let $\mathcal R_h^q$ restrict a smooth scalar field to the pressure-test labels.
The discrete consistency requirement is
\begin{equation}
 e_{D,h}(w)=
...
\end{equation}
for smooth $w$ and a regular cloud family.  Case~3 measures
\eqref{eq:discrete_terminal_tangent_consistency} directly, including its
boundary-layer decomposition.

Pressure elimination also requires consistency of the composite response.
For a smooth pressure $q$, let $I_h^Qq$ denote its coefficient samples and set
\begin{align}
 \varepsilon_{G,h}(q)
...
\end{align}
The norm of the sum must tend to zero independently of
\eqref{eq:discrete_terminal_tangent_consistency}.  Second-order action accuracy
in $M_U$ alone does not imply second-order accuracy after differentiation:
an $O(h^{-1})$ discrete derivative bound permits an $O(h)$ propagated error.
Case~3 measures both terms of \eqref{eq:discrete_composite_consistency}.

CCOP therefore controls a represented terminal pressure-test tangent, not
the derivative of an asserted discrete volume constraint.  The residual
$D_{T,h}^qu_h^T$ is controlled by the pressure solve, whereas $J_h-1$, $A_J$,
and $A_\Gamma$ also depend on spatial and temporal discretization.  In
particular, finite-step state error can persist in a spatially exact affine
subspace.  Direct Jacobian closure and target--action assignment address
different parts of the discrete problem.

This is the point at which the finite-step constraint viewpoint meets the
projection architecture.  The accepted state fixes the terminal constraint
tangent $D_T$, but retaining a pressure/PPE factorization represents the
multiplier through a pressure field and a continuum action operator.  Unlike an
abstract finite-dimensional reaction direction, that action is the
configuration-dependent Piola map $G[q]$.  The pressure block of the
factorization therefore depends on two geometries: the terminal geometry at
which admissibility is tested and the action geometry at which the multiplier
is represented.

At frozen geometry the pressure derivative of the terminal tangent residual is
\begin{equation}
 D_p\!\left[\D_T
...
\end{equation}
Hence $\D_T\G_\Th$ is the frozen pressure-to-terminal-tangent response.  In the
coupled step, the pressure dependence of $x_T$ adds the corresponding geometry
blocks to this principal response.

\subsection{Why the constraint is terminal}
...
\subsection{Why the pressure action is placed}
...
\end{equation}
The action configuration therefore defines a one-stage representative of the
pressure impulse, whereas the terminal configuration defines an admissibility
target.  The temporal consequence of this freedom follows from the usual
one-point quadrature expansion, provided the pressure stage and the geometric
path are themselves consistent.

\begin{proposition}[Defect of the placed pressure impulse]
\label{prop:placed_impulse_order}
Let $g(t)=\G(t)p(t)$ be twice continuously differentiable on one force-free
pressure substep, and define the action-stage error in the momentum norm by
\begin{equation}
...
$O(\dt)$ endpoint stage error has the same local order.  If the remaining
stages are stable and have matching local consistency, standard one-step
accumulation gives global orders two and one for the corresponding pressure
paths.  This last implication is conditional; it is not a global convergence
theorem for an arbitrary force, viscous, or boundary predictor.
\end{proposition}
...
\eqref{eq:placed_impulse_defect}.  The stated orders follow by cancellation
at $\Th=1/2$ and standard stable accumulation of the local defects.
\end{proof}

The preceding statement isolates quadrature from stage construction.  The
next result closes that distinction for the actual CCOP equations on a fixed
spatial discretization.

\begin{proposition}[Local defect of the force-free CCOP step]
\label{prop:actual_ccop_local_defect}
Fix finite-dimensional velocity, pressure, and constraint spaces and consider
the smooth semidiscrete constrained system
\begin{equation}
 \dot x=u,\qquad \dot u=-G(x)p,\qquad D(x)u=0.
 \label{eq:semidiscrete_constrained_flow}
\end{equation}
Suppose $D,G$, and the pressure branch are twice continuously differentiable
near an exact state $(x_n,u_n,p_n)$, and suppose
$L_n=D_nG_n$ is bijective on the chosen pressure quotient with
$\norm{L_n^{-1}}\le C_L$.  Starting from that exact state, apply
\eqref{eq:nonlinear_velocity}--\eqref{eq:nonlinear_terminal_constraint} with
$u^\star=u_n$, $x^\star=x_n+\dt u_n$, $\gamma=1/2$, and
$x_\Th=x_n+\Th(x_T-x_n)$.  Then, in any fixed equivalent finite-dimensional
norm,
\begin{equation}
 u_{T,\Th}^h-u_h(t_n+\dt)
 =\left(\frac12-\Th\right)\dt^2
   \Pi_n G_n'[u_n]p_n+O_h(\dt^3),
 \qquad
...
\begin{proof}
Differentiating the exact constraint at $t_n$ gives
$D_nG_np_n=D_n'[u_n]u_n$.  The $O(\dt)$ term of the numerical terminal
constraint gives the same equation for the leading numerical multiplier;
bijectivity of $D_nG_n$ therefore permits the expansion
$p_\Th^h=p_n+\dt\pi_1+O_h(\dt^2)$.  Trapezoidal kinematics gives
$x_T=x_n+\dt u_n-\tfrac12\dt^2G_np_n+O_h(\dt^3)$, and hence
\begin{equation*}
 G(x_\Th)p_\Th^h
 =G_np_n+\dt\bigl(G_n\pi_1+\Th G_n'[u_n]p_n\bigr)
  +O_h(\dt^2).
\end{equation*}
The exact semidiscrete velocity has the expansion
\begin{equation*}
 u_h(t_n+\dt)=u_n-\dt G_np_n
 -\frac{\dt^2}{2}\bigl(G_n\dot p_n+G_n'[u_n]p_n\bigr)
 +O_h(\dt^3).
\end{equation*}
The exact and numerical terminal positions agree through $O_h(\dt^2)$.
Subtracting their terminal constraints therefore shows that the coefficient
of $\dt^2$ in the velocity difference lies in $\kernel D_n$.  With
$\delta p=\pi_1-\dot p_n/2$, this condition is
\begin{equation*}
 D_nG_n\delta p
 =-\left(\Th-\frac12\right)D_nG_n'[u_n]p_n.
\end{equation*}
Solving on the pressure quotient and substituting into the velocity expansion
gives \eqref{eq:actual_ccop_local_velocity_defect}.
\end{proof}

The proof also identifies the multiplier stage:
\begin{equation}
 p_\Th^h
 =p_n+\frac{\dt}{2}\dot p_n
  -\left(\Th-\frac12\right)\dt
   (D_nG_n)^{-1}D_nG_n'[u_n]p_n
  +O_h(\dt^2).
 \label{eq:actual_ccop_multiplier_stage}
\end{equation}
Thus the midpoint coefficient represents both the instantaneous midpoint
pressure and the step-average pressure through first order; generic endpoint
coefficients differ from their endpoint samples at $O(\dt)$.  This is a
consequence of the terminal constraint solve, whereas centering the geometric
operator remains the independent choice controlled by $\Th$.
...
cancels the $O(\dt^2)$ configuration term for the actual force-free pressure
solve, not merely for an externally prescribed sample of $g(t)$.  Relative to
...
\end{equation}
where $r$ is established by the spatial action audit.  External-force,
viscous, splitting, and nonhomogeneous-boundary defects require their own
matching-order predictor estimates and are not covered by
\cref{prop:actual_ccop_local_defect}.

Proposition~\ref{prop:placed_impulse_order} is both stronger and narrower than calling $\Th$ an
unqualified method order.  It proves a pressure-substep design rule under the
stage-consistency hypothesis.  If the placed pressure, configuration path,
nonpressure predictor, viscous treatment, or force splitting gives only
$\eta_{1/2}^h=O(\dt)$, that term restores an $O(\dt^2)$ local defect even at
$\Th=1/2$.  Conversely, changing $\Th$ in the present algorithm changes only
the action geometry: it adds no pressure unknown, Runge--Kutta stage, or
pressure equation per nonlinear geometry iteration.

The target/action roles lead to
\begin{equation}
...
Let $g_\Th=\G_\Th p_\Th$ and
$E_m(u)=\half\norm[m]{u}^2$.  Independent of any adjoint relation,
\begin{equation}
 \Delta E_p(\Th)
 =E_m(u_T)-E_m(u^\star)
 =-\dt\ip[m]{u^\star}{g_\Th}
  +\frac{\dt^2}{2}\norm[m]{g_\Th}^2.
 \label{eq:pressure_work_identity_continuum}
\end{equation}
Terminal closure therefore fixes neither the pressure impulse nor its kinetic
work.

Under the stronger same-configuration adjoint assumptions, the two endpoint
placements have a useful sign interpretation.
...
Substitution into \eqref{eq:pressure_work_identity_continuum} proves the
claim.
\end{proof}

Whenever the endpoint values bracket zero, continuity supplies a work-neutral
interior placement; this is the criterion used in the experiment.  More
generally, the configuration assignment is summarized by
\begin{equation}
...
nonpressure predictor supplies $u^\star$ and $x^\star$.  CCOP restricts the
unresolved multiplier-action geometry to the affine labelled-position path
between the two step endpoints,
\begin{equation}
  x_\Th=x_n+\Th(x_T-x_n),\qquad 0\le\Th\le1.
  \label{eq:linear_action_path}
\end{equation}
This path is an explicit one-point action model, not an assertion that the
numerical method resolves a unique substep trajectory.  Its role is to keep
every placement inside the space of compatible material configurations while
retaining a single scalar pressure/PPE solve.  With the current spatial
gradient $\Grad_n$,
\begin{align}
...
\end{equation}
The value $\gamma=1/2$ is fixed by trapezoidal particle kinematics rather than
tuned to the work-neutral placement.  In the force-free case
...
kinematic relation into $x^\star$.  Matching-order force and viscous stages
preserve the pressure-step cancellation in
\cref{prop:placed_impulse_order}.  The projector introduced below is the
frozen linear algebra embedded in this nonlinear geometry coupling.
...
cancels the leading impulse-quadrature term, whereas terminal placement
retains it.  The same conclusion extends to a complete scheme when its force
and viscous stages satisfy the consistency condition
\eqref{eq:second_order_stage_consistency}.
...
Since $b_\Th$ contains $\dt^{-1}$, the generic small-step contraction scales
as $O(\dt)$.  In the discrete calculations, the measured Riesz gap supplies
...
The row-restricted $D_T^q$ is a represented pressure-test operator, not the
derivative of the reported polygonal or Jacobian area.  Consequently,
$D_T^qu_T=0$ is an exact algebraic statement in $Q_{h,0}^*$, while
...
\end{equation}
then the response is formed on the quotient rather than by fixing a pressure
node.  No periodic pin, deleted divergence row, spectral mode filtering, or
diagonal regularization is used.
...
of $D_h$ and $G_h$ does not determine the cross response.
...
complete current pressure-action state; a cumulative Jacobian is not
...
\section{Case 1: Terminal closure and pressure work are independent finite-step properties}
\label{sec:result1}
...
\subsection{The anchor determines which constraint is closed}
...
action ranges.  The third projector panel includes the auxiliary compatible
endpoint control, which removes the meshless-pair nonorthogonality at
$\Th=T$.}
\label{fig:test1_mechanism}
\end{figure}

\subsection{The placed action determines projector geometry and pressure work}
...
kinetic identity \eqref{eq:pressure_work_identity_continuum} closes at the
floating-point arithmetic floor.
...
$\norm[M_U]{\Pi_T}=1.61863$, with relative Green/adjoint defect 0.248.
...
The endpoint adjoint defect is a full-space measure.  In this affine test, the
trace-free velocity and quadratic pressure lie in the exactly reproduced
polynomial subspace, where mapped action errors are below $10^{-15}$.  This
resolved subspace explains the nearly antisymmetric endpoint work values.
...
\section{Case 2: The configuration choice accumulates in long-time affine motion}
\label{sec:result2}

Case~1 identifies the one-step mechanism.  The second case asks whether that
finite-step bias accumulates into measurable phase and energy errors when
spatial reconstruction is deliberately prevented from setting the accuracy
floor.  We use the two-dimensional conservative affine drop under a central
potential force.  The initial circular drop has $R=0.5$, the forcing frequency
is $\Omega=1.2$, and the velocity field is affine,
\begin{equation}
...
archived driver is specified in \cref{sec:case2_implementation_contract}; this
case is not used to infer a spatial rate.  Writing the principal semi-axes as
...
$T=30$.  This construction makes the case a long-time test of the pressure
update rather than of spatial approximation.

\subsection{A same-discretization ISPH workflow comparison}

We compare two pressure workflows at $R/\Delta x=50$ using the same
quadratic material-derivative discretization.  The archived \texttt{SPH2}
driver uses a cubic SPH kernel to weight a quadratic MLS fit; the \texttt{MLS2}
and \texttt{SPH2} strict drivers share this derivative construction and are not
treated as independent spatial methods.  The pressure is a general nodal field
with homogeneous Dirichlet boundary values, not a prescribed quadratic ansatz.
...
\end{equation}
The second operator acts on native physical-pressure coefficients and contains
no $J_\alpha$ factor.  This convention is identical in both branches.  For a
spatially uniform affine Jacobian, multiplying $G_\alpha^{\rm phys}$ by
$J_\alpha$ leaves its range and the frozen projector unchanged, but rescales
the pressure coefficient.  Such equivalence is not asserted for nonaffine maps.
...
are shared with CCOP.  This is an implementation-level comparison of the
ISPH workflow on the present discretization, not a reproduction of every
variant of ISPH or a comparison with a density/Jacobian state constraint.
The fixed star--star system is solved by sparse LU.  Its own-anchor residual
is accepted independently of $D_T^qu_\star^T$, which is retained as a common
terminal diagnostic without an extra projection.

CCOP instead couples the terminal target to the midpoint action.  For a
...
the kinematic fixed-point defect.  This branch is a pressure-only solver
variant, not the outer block-Picard implementation used in Cases~1 and~3;
details are recorded in \cref{sec:case2_implementation_contract}.

\begin{figure}[t]
\centering
\safegraphic[0.96\linewidth]{fig_case2_long_time_axes_pressure_energy.pdf}
\caption{\textbf{Long-time affine accuracy of midpoint pressure action.}
(a) Major and minor semi-axes against the affine reference.
(b) Accumulated timing error of successive positive deformation maxima.
(c) Relative total-energy error for $\Delta t=2\times10^{-3}$ and
$10^{-3}$.  (d) Kinetic, potential, and total conservative energy exchange.
(e) Centre-pressure error evaluated at the midpoint action time.
(f) Error reduction when the time step is halved; the dashed line marks the
factor four expected from second-order behavior.}
\label{fig:case2_affine_transfer}
\end{figure}

Figure~\ref{fig:case2_affine_transfer} shows that the midpoint branch follows
the analytic semi-axes and centre pressure over eight oscillations.  Halving
the time step reduces the major-axis, minor-axis, signed-shape, energy, centre-
pressure, and accumulated phase errors by factors close to four.  Kinetic and
conservative potential energy exchange periodically, while the total-energy
error remains bounded within each cycle and returns to the $10^{-10}$ scale at
$T=30$.
...
Table~\ref{tab:case2_transfer_numbers} compares the two workflows on the same
spatial discretization.  The ISPH (star--star) branch solves its own anchor to the
algebraic floor but retains a terminal residual of $5.36\times10^{-7}$ and a
$4.31\times10^{-3}$ energy drift.  At the same step size, midpoint CCOP closes
the terminal target to $1.78\times10^{-14}$ and reduces the maximum energy
error to $1.45\times10^{-7}$.  The full-period error decreases from
$2.52\times10^{-4}$ for star--star to $1.38\times10^{-6}$ for midpoint CCOP.
For midpoint CCOP, halving the time step gives observed two-level orders
$1.998$ for both semi-axes, $2.015$ for the RMS energy error, $2.044$ for the
centre pressure, and $2.014$ for the accumulated phase error.  The affine
reference therefore gives a long-time state, pressure, phase, and energy
comparison in a polynomially reproduced setting.  This pair changes both the
anchor and the action placement; their individual effects are separated in
Case~1, not inferred separately from this long-time pair.  Equal pressure
dimension does not imply equal cost for the frozen LU and nonlinear solves.

\subsection{Relation to other particle formulations}
...
\section{Case 3: Configuration separation persists under nonaffine material shear}
\label{sec:result3}
...
\subsection{Boundary-action propagation limits composite consistency}
...
an $O(h)$-thick boundary region.  Localization alone does not explain the
approximately second-order solution errors: for a regional RMS error of
order $h^r$, an $O(h)$ mass fraction contributes only a factor $h^{1/2}$
to its global RMS norm.  The error propagated through the inverse response
in \eqref{eq:discrete_response_transfer} must be distinguished from this
operator-action error.  Thus the present data establish a slower composite
consistency rate, alongside the separately observed smooth-vortex solution
rate.  They identify boundary propagation of the placed action as the leading
spatial mechanism limiting the composite response.

\subsection{The inverse response retains accurate smooth pressure actions}
...
\end{equation}
The right-hand side is the analytic composite action, not $A_hI_h^Qq$.
No particle, stencil, pressure space, or operator is changed.  For the physical
...
the two terms are combined; it is not explained solely by cancellation in the
final error or contraction of the boundary region.
...
response on the tested smooth fields; they do not assert second-order
$DG$ consistency for arbitrary pressure data or a uniform theorem over
all shears and placements.
...
\subsection{Complete nonaffine convergence}
...
deformation.  The regional decomposition also singles out boundary pressure
action as the next component to validate on a moving free surface.
...
accumulated numerical state and includes the conservative-force predictor,
whereas that proposition is an exact-start force-free fixed-space result.
...
pressure-space dimension or introducing another pressure stage.
...
midpoint placement to the pressure-impulse defect estimate.  In long-time
affine motion, the same-discretization ISPH star--star comparison then shows
how accurate predictor closure can coexist with accumulated terminal mismatch.
...
Discrete closure is in the selected pressure-test space, not an assertion of
exact Jacobian closure.  Midpoint temporal accuracy requires stage consistency,
and the sampled response gaps are finite-dimensional stability evidence.  The
transported Piola rows converge on constant physical velocities but do not
annihilate them algebraically on a finite nonaffine cloud; the resulting
translation-equivariance defect is therefore reported as a spatial
consistency error rather than hidden by an additional projection.
...
effect is not visible in the terminal residual alone: equally accurate closure
can accompany different finite-step pressure-work and deformation paths.
...
uses terminal divergence and midpoint physical-pressure action; the available
\texttt{terminal-terminal} branch uses terminal divergence and terminal action.
Only the first two have long-time histories in
\cref{tab:case2_transfer_numbers}.  Selecting an action position alone is not
equivalent to selecting the star--star workflow, because its target must also
be $D_\star^q$.
...
$\norm[M_U]{\Pi_T}=1.61863$ and a measured mass-adjoint defect 0.2484,
confirming that endpoint orthogonality is unavailable for this pair.
