...
accuracy.  A fixed-space local-branch expansion of the actual CCOP equations
...
actual CCOP residual is degenerate at zero step size: if $D_nu_n=0$, then
...
open set $j_Tj_\Th\ne0$.  The raw-determinant acceptance test verifies that the
reported states remain in this set; it is not the source of analyticity.

The lemma assumes exact represented closure of the incoming state.  If
$D_nu_n=\epsilon\ne0$, the rescaled equation contains $\epsilon/\tau$ and the
same zero-step expansion is not uniform.  Full-operator commit carries the
accepted terminal residual into the next step, while the remaining
floating-point residual is recorded as a numerical perturbation rather than
an analytic existence condition.

The lemma identifies the unique local solution branch; it does not assert that
a particular nonlinear iteration reaches that branch.  The conditional
convergence of the block Picard map is treated separately in
\cref{thm:local_coupled_closure}.
...
coefficients differ from their endpoint samples at $O(\tau)$.  This is a
...
conservative-force architecture used in Cases~2 and~3.  Velocity-dependent
viscous terms and split operators require a separate stage-consistency
analysis.
...
\eqref{eq:second_order_force_position_predictor}, not merely for an externally
prescribed sample of $g(t)$.  Relative to
...
nonpressure predictor, or split stage gives only
