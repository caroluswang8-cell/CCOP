...
\subsection{A same-discretization three-branch comparison}

We compare three configuration assignments at $R/\Delta x=50$ using the same
...
The terminal--terminal control retains the nonlinear terminal target but places
the pressure action on that same endpoint.  It solves
\begin{equation}
 D_T^q[x_T]\bigl(u^\star-\Delta t\,G_T^{\rm phys}[x_T]p_T\bigr)=0,
 \qquad
 x_T=x^\star-\frac{\Delta t^2}{2}G_T^{\rm phys}[x_T]p_T.
 \label{eq:case2_terminal_terminal_control}
\end{equation}
It therefore separates the consequence of terminal represented closure from
the consequence of pressure-action placement without changing the pressure
space or terminal geometry solve.
...
\caption{\textbf{Long-time separation of terminal closure and pressure-action placement.}
...
(f) Common terminal-divergence diagnostic.  Star--star, terminal--terminal,
and midpoint use the same particles, pressure space, material derivatives,
force predictor, and position update.  The three midpoint curves use
$\Delta t=4\times10^{-3},2\times10^{-3},10^{-3}$; both controls use
$\Delta t=2\times10^{-3}$.}
\label{fig:case2_affine_transfer}
\end{figure}

Figure~\ref{fig:case2_affine_transfer} separates two effects that are hidden in
a two-branch comparison.  Terminal--terminal reduces the common terminal
diagnostic from $5.36\times10^{-7}$ for star--star to
$2.21\times10^{-14}$, yet its semi-axis, centre-pressure, and energy histories
remain nearly coincident with star--star.  Terminal represented closure alone
therefore does not select the finite-step pressure-work trajectory.  Moving the
action to the midpoint changes that trajectory: at the same time step, the
maximum mechanical-energy error decreases from $4.31\times10^{-3}$ to
$1.45\times10^{-7}$.  Across the three midpoint step sizes, the observed
...
\eqref{eq:case2_same_discretization_isph}; terminal--terminal denotes the
endpoint-action control in \eqref{eq:case2_terminal_terminal_control}.}
...
Terminal--terminal & $2{\times}10^{-3}$ & $2.21{\times}10^{-14}$ & $2.21{\times}10^{-14}$ & $2.40{\times}10^{-3}$ & $2.27{\times}10^{-3}$ & $4.31{\times}10^{-3}$ & $1.93{\times}10^{-3}$ \\
...
diagnostics.  The star--star/terminal--terminal comparison changes the terminal
target while leaving endpoint pressure action, long-time state error, and
energy drift essentially unchanged.  The terminal--terminal/midpoint comparison
then retains terminal closure while changing the action placement.  Together
these controls carry the one-step separation in Case~1 into a long-time
calculation without attributing the improvement to a different particle cloud,
pressure space, or derivative formula.  Equal pressure dimension does not imply
equal cost for the frozen LU and nonlinear solves.
