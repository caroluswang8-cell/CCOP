...
\begin{abstract}
Standard semi-implicit Lagrangian projection methods evaluate incompressibility and pressure action on one geometric stage. We show that this common-stage construction hides a configuration-assignment problem. The accepted velocity constraint belongs to the terminal configuration, whereas the pressure action belongs to a separately selected material configuration. We formulate this separation as configuration-consistent oblique projection (CCOP), with cross response \(D_TG_\Theta\) and one scalar pressure equation. The parameter \(\Theta\) is geometric rather than merely temporal: it assigns the pressure operator without moving the terminal incompressibility target. Every admissible placement satisfies an exact discrete pressure work--energy identity with its represented action, while the placement determines the value and physical accuracy of that work. We establish the projection structure, relative-deformation solvability, and a local branch of the coupled step. On a fixed spatial discretization, midpoint action placement cancels the leading configuration-induced velocity defect for the force-free step and the second-order conservative-force predictor used here. Affine tests isolate terminal closure, projector obliqueness, and pressure work; a matched ISPH comparison shows their long-time consequences. A strictly nonaffine differential vortex verifies transported operators and complete evolution under spatially varying material shear. CCOP therefore makes the finite-step relation between terminal closure and pressure work explicit without adding a pressure stage.
\end{abstract}
...
\]
The parameter $\Theta$ assigns an operator to a material geometry; it is not,
by itself, a temporal stage of the state integrator or of the pressure
coefficient.  In particular, midpoint action produces the mixed response
$D_TG_{1/2}$, not the common-stage response $D_{1/2}G_{1/2}$.  These responses
coincide only under additional stage-consistency assumptions.
...
the leading configuration-induced velocity defect.  The numerical comparison is
...
target.  Three choices are distinct in a finite material step: the temporal
stage of the state integrator, the stage represented by the pressure
coefficient, and the material configuration carrying the pressure operator.
The parameter $\Th$ controls only the third choice.  Its temporal consequence
can be quantified by a one-point quadrature expansion once the coefficient
stage and geometric path satisfy their own consistency conditions.
...
This proposition is an accuracy statement for the represented impulse, not
the source of the exact work identity established below.  It isolates
quadrature from pressure-stage construction and geometric operator placement.  The
...
The pressure correction also carries an exact finite-step work identity.  This
identity concerns consistency between the represented pressure action and the
momentum change that it generates.  It is distinct from the temporal accuracy
with which that action approximates the continuous pressure work.
...
the pressure-induced kinetic-energy change.  The identity requires neither
...
consistency with the represented pressure action.  It does not assert that
$W_{p,\Th}^{\dt}$ is a high-order approximation of the physical pressure work
\begin{equation}
 W_p^{\rm ex}
 =-\int_{t_n}^{t_{n+1}}\ip[m]{u(t)}{\G(t)p(t)}\,dt.
 \label{eq:exact_continuous_pressure_work}
\end{equation}
That approximation depends on the geometric action placement, pressure-stage
consistency, the nonpressure predictor, and any force or viscous splitting.
Accordingly, $D_T$ selects the represented admissible terminal state, whereas
$G_\Th p_\Th$ selects the pressure direction and the work realized by the
correction.  Terminal closure alone fixes neither the pressure impulse nor its
kinetic work.
...
\end{equation}
This path assigns the pressure operator to a material configuration; it does
not prescribe a temporal stage for every field and does not assert that the
method resolves a unique substep trajectory.  Hence $\Th=1/2$ defines the
mixed-configuration response $D_TG_{1/2}$ rather than a common-stage midpoint
projection.  Its role is to keep every placement inside the space of
compatible material configurations while retaining a single scalar
